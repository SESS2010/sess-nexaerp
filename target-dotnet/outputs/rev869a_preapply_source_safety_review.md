...
## 2026-08-10 approved UOM decision superseding the pending-state addendum

Management approval `MGMT-REV869A-UOM-20260810-001` resolves the prior UOM data-readiness blocker source-only. The approved creation set contains exactly one migration-owned UOM: ID `f71a4725-bb15-e7bf-e97b-991985e96328`, code `EA`, name `Each`, dimension `COUNT`, precision `0`, canonical base `true`, conversion policy `IDENTITY_ONLY`, lifecycle `CREATE`, approval `APPROVED`. The approved mapping set contains exactly Item `8c428e59-db05-471d-a7e7-4f7dc1c13b54` / `REV868C1-ITEM`, with both `UomId` and `BaseUomId` set to that EA ID.

Preflight remains pre-apply and SELECT-only: it requires zero pre-existing ID/code/name collisions, exact item identity and attributes, one approved mapping, and zero uncovered null-UOM items. It no longer deadlocks by demanding the migration-owned EA row before Up. Any additional null item, missing/extra/duplicate/invalid mapping, or inferred/default value fails closed.

The unchanged migration ID is `20260810120000_Rev869AIdentityMasterScopeFoundation`. Up backs up all Items/UOMs/Vendors before mutation, inserts one EA row, updates one approved Item, preserves the complete management contract in one immutable controlled-configuration history row, and retains the 88-row security/configuration contract and Department Manager reuse. Down proves exact backed-up state, restores the Item's original null UOM, deletes only the exact migration-owned history and EA rows, and drops backup tables last. PostgreSQL apply and acceptance remain unclaimed and require a future approved isolated run.