...
One Gate Entry produces one effective GRN. Gate Entry records both the arrival header and delivered quantities by PO line. Over-receipt is prohibited: the total effective GRN quantity for a PO line may never exceed the PO ordered quantity. If 105 arrives against an ordered quantity of 100, the GRN receives 100 and records the excess 5 as rejected for return to the vendor. There is no manager override for over-receipt. Received-to-date is computed from effective finalised GRN lines; it is not stored on the PO line.

| Step | Actor | Action | Approval / notification | Document produced | Schema support |
|---:|---|---|---|---|---|
| 1 | Stores today; security later | Creates a separate Gate Entry linked to one PO, recording vendor DC number, vehicle number, mode of transport, date/time, received by, ISO receipt-verification details, and delivered quantities by PO line. Multiple Gate Entries may link to the same PO; each Gate Entry produces one effective GRN. | None stated. | Gate Entry and delivered PO-line quantities | **NO SCHEMA SUPPORT TODAY**. |
...
| 5 | KAMALI, SUDALAI, or KARTHICK | Enters GRN in either company against the mandatory Gate Entry. No vendor bill means no GRN. The GRN records vendor name, bill number, bill date, vendor DC number, mode of transport, and ISO receipt-verification details. Each GRN line snapshots HSN, GST percentage, model, manufacturer part number, UOM, and the relevant Item identity at entry. Later Item-master changes do not alter historical GRNs. | Mandatory bill, Gate Entry, and snapshot controls. | GRN and immutable line snapshots | **NO SCHEMA SUPPORT TODAY**. |
| 5A | ERP / Stores | Limits the effective received quantity to the PO ordered quantity after subtracting earlier effective finalised GRNs. Any delivered excess is recorded as rejected for vendor return and is not received into stock. There is no manager override. | Hard block against over-receipt. | GRN excess-rejection evidence | **NO SCHEMA SUPPORT TODAY**. |
| 6 | KAMALI, SUDALAI, or KARTHICK | Captures per-unit serial when item value is above the configured 5,000 threshold unless Item override applies. At or below the threshold, serial is optional. On a duplicate serial, ERP warns and requires the operator to make the stored value unique before finalising, for example by appending financial year and make as `A12345/2026-27/BITZER`. | Warning during entry; unique stored serial is mandatory at finalisation. | Unit serial record | **NO SCHEMA SUPPORT TODAY** for GRN serials, threshold evaluation, duplicate warning, disambiguation, and uniqueness enforcement. |
| 7 | ERP / responsible user | Records warranty once on the GRN line, not once per physical unit. Bill date is mandatory. Installation date comes from the installation completion report and, until an Installation module exists, is entered manually on the Job Order. Warranty expiry is the earlier of 12 months from installation or 13 months from bill date; if installation date is absent, use 13 months from bill date. | Mandatory warranty rule. | Line-level warranty dates and expiry | **NO SCHEMA SUPPORT TODAY** for this GRN/Job Order warranty linkage. |
...
For job work such as CNC, powder coating, and milling, the returnable DC must record dispatched weight and the receipt must record returned weight. The difference is subcontract scrap and the vendor must account for it. For example, if 10 kg of stainless steel is sent and 8 kg is returned, the vendor must explain the 2 kg difference. This is a settled requirement for the future Subcontract module and is not part of the first Stores module.
...
| 2 | Stores | Sends material on returnable DC with expected return date and mandatory dispatched weight. | Approved request required. | Outstanding DC with dispatched weight | **NO SCHEMA SUPPORT TODAY**. |
...
| 4 | Stores | Receives material back and records mandatory returned weight. ERP calculates dispatched weight minus returned weight as scrap requiring vendor explanation. | Vendor accountability evidence required for any difference. | Return receipt and weight-accounting evidence | **NO SCHEMA SUPPORT TODAY**. |
...
- How are damaged, free, and other receipt exceptions handled beyond the stated partial-delivery and no-over-receipt rules?
...
- How are outgoing material, processed return, vendor PO, QC result, and project/Job Order linked for subcontract beyond the settled dispatched-weight, returned-weight, calculated-scrap, and vendor-explanation requirement?
...
| 21 | Gate Entry and GRN | Schema | **New** | Separate Gate Entry document with one-PO ownership, delivered PO-line quantities, and one effective GRN; many Gate Entries per PO; mandatory bill and Gate Entry reference on GRN; GRN-line snapshots of HSN/GST/model/manufacturer part number/UOM; no-over-receipt constraint and excess-rejection evidence; serial threshold/override and unique disambiguated values; line-level warranty, bill date, manual Job Order installation date, warranty fallback/expiry, reversals, and histories. |
| 22 | Gate Entry and GRN | Service/API | **New** | Create PO-linked Gate Entries; block GRN without Gate Entry, Item, or vendor bill; support partial deliveries through multiple Gate Entries; hard-block receipt beyond PO ordered quantity with no override path and record delivered excess for return; freeze Item snapshots; generate/print ERP barcodes; warn on duplicate serial and require a unique stored value before finalisation; calculate line-level warranty with manual installation date or 13-month fallback; expose Gate/GRN endpoints. |
...
| 31 | Subcontract | Schema/service/API | **Advance plus new** | Reuse vendor quotation and PO; add material-out/DC with dispatched weight, material-return with returned weight, calculated scrap difference and mandatory vendor explanation, mandatory QC, PO-bill value, BOM allocation, workflow services, and endpoints. |
