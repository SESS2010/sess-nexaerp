...
| `POST /tax-gst` | `TaxGstRequest` below plus required `Idempotency-Key` header | `201 TaxGstWorkflowResult` | `settings.tax-gst:Create`; exact `ACCOUNTS_MANAGER` | creates `Pending Approval`; invalid split/overlap/supersession `400/409` |
| `POST /tax-gst/{id}/approve` | `{ "ExpectedVersion":0, "Remarks":"GST portal cross-check complete", "IdempotencyKey":"..." }` | `200 TaxGstWorkflowResult` | `settings.tax-gst:Approve`; exact `TECHNICAL_DIRECTOR` or `MANAGING_DIRECTOR` | creator self-approval, stale/replayed/non-pending decision `403/409` |
| `POST /tax-gst/{id}/reject` | `{ "ExpectedVersion":0, "Remarks":"Reason", "IdempotencyKey":"..." }` | `200 TaxGstWorkflowResult` | `settings.tax-gst:Reject`; exact `TECHNICAL_DIRECTOR` or `MANAGING_DIRECTOR` | reason required; creator self-rejection, stale/replayed/non-pending decision `403/409` |
