...
internal static class Rev869BDatabaseSafetySql
{
    ...
    private const string InstallTemplate = """
        ...
        RETURNS boolean LANGUAGE plpgsql SET search_path = pg_catalog, __advance_schema__ AS $rev869b$
        DECLARE
        ...
        BEGIN
            ...
            SELECT ql."Quantity" quantity, ql."UnitRate" unit_rate,
              ...
              JOIN __advance_schema__.tax_gst_settings t ON t."Id"=ql."TaxGstSettingId"
        ...
        CREATE OR REPLACE FUNCTION __advance_schema__.rev869b_guard_child_insert()
        ...
        BEGIN
            IF TG_TABLE_NAME='request_for_quotation_lines' THEN
                SELECT count(*) INTO matched FROM __advance_schema__.request_for_quotations r
            ...
            ELSIF TG_TABLE_NAME='vendor_quotation_lines' THEN
                SELECT count(*) INTO matched FROM __advance_schema__.vendor_quotations q
            ...
            ELSIF TG_TABLE_NAME='commercial_comparison_lines' THEN
                SELECT count(*) INTO matched FROM __advance_schema__.commercial_comparisons c
            ...
            ELSIF TG_TABLE_NAME='purchase_order_lines' THEN
                SELECT count(*) INTO matched FROM __advance_schema__.purchase_orders p
                 WHERE p."Id"=NEW."PurchaseOrderId" AND p."Status" IN ('Draft','RevisionDraft')
                   AND p."Version"=0 AND length(trim(p."OrganizationId"))>0 AND
                   (p."IsCurrentVersion" OR
                    (NOT p."IsCurrentVersion" AND p."PreviousVersionId" IS NOT NULL AND
                     p.xmin::text::bigint=txid_current()));
            ELSIF TG_TABLE_NAME='quotation_technical_verifications' THEN
                SELECT count(*) INTO matched FROM __advance_schema__.vendor_quotation_lines ql
                JOIN __advance_schema__.vendor_quotations q ON q."Id"=ql."VendorQuotationId"
            ...
            ELSE RAISE EXCEPTION USING ERRCODE='P0001',SCHEMA='__advance_schema__',TABLE=TG_TABLE_NAME,
              CONSTRAINT='rev869b_guard_child_insert',MESSAGE='Unsupported REV869B controlled child relation.';
            END IF;
            IF matched<>1 THEN RAISE EXCEPTION USING ERRCODE='P0001',SCHEMA='__advance_schema__',TABLE=TG_TABLE_NAME,
              CONSTRAINT='rev869b_guard_child_insert',
              MESSAGE='REV869B child INSERT requires exactly one editable parent version.'; END IF;
            RETURN NEW;
        END $rev869b$;

        CREATE OR REPLACE FUNCTION __advance_schema__.rev869b_qualification_provenance_valid(qualification_id uuid)
        RETURNS boolean LANGUAGE sql STABLE SET search_path=pg_catalog,__advance_schema__ AS $rev869b$
        SELECT EXISTS (
          SELECT 1 FROM __advance_schema__.vendor_qualifications q
          WHERE q."Id"=qualification_id AND q."VerificationStatus" IN ('Verified','Approved') AND q."ApprovalStatus"='Approved'
            ...
            AND (SELECT count(*) FROM __advance_schema__.controlled_configuration_histories h
                 JOIN __advance_schema__.employee_identity_mappings m ON m."OrganizationId"=q."OrganizationId" AND m."Subject"=h."ActorLoginId"
        ...
        CREATE OR REPLACE FUNCTION __advance_schema__.rev869b_guard_authoritative_transition()
        RETURNS trigger LANGUAGE plpgsql SET search_path = pg_catalog, __advance_schema__ AS $rev869b$
        ...
        BEGIN
            IF TG_TABLE_NAME='commercial_comparisons' AND NEW."Status" IN ('PendingApproval','Approved') THEN
                SELECT count(*) INTO expected_count FROM __advance_schema__.vendor_quotation_lines ql WHERE ql."VendorQuotationId"=NEW."RecommendedVendorQuotationId";
                SELECT count(*) INTO matched_count FROM __advance_schema__.commercial_comparison_lines cl
                JOIN __advance_schema__.vendor_quotation_lines ql ON ql."Id"=cl."VendorQuotationLineId"
                JOIN __advance_schema__.vendor_quotations q ON q."Id"=ql."VendorQuotationId"
                JOIN __advance_schema__.rfq_vendor_invitations i ON i."Id"=q."RfqVendorInvitationId"
                JOIN __advance_schema__.request_for_quotation_lines rl ON rl."Id"=ql."RequestForQuotationLineId"
                JOIN __advance_schema__.items item ON item."Id"=rl."ItemId"
                JOIN __advance_schema__.uoms uom ON uom."Id"=item."BaseUomId"
                JOIN __advance_schema__.vendors vendor ON vendor."Id"=q."VendorId"
                JOIN __advance_schema__.vendor_qualifications qualification ON qualification."VendorId"=vendor."Id"
                 ...
                 WHERE cl."CommercialComparisonId"=NEW."Id" AND cl."IsRecommended" AND
                       ...
                       qualification."IsActive" AND qualification."VerificationStatus" IN ('Verified','Approved') AND qualification."ApprovalStatus"='Approved' AND
                       ...
                       __advance_schema__.rev869b_qualification_provenance_valid(qualification."Id") IS TRUE AND
                       ...
                       __advance_schema__.rev869b_commercial_snapshot_reconciles(ql."Id",cl."CommercialSnapshotJson",ql."TaxRuleSnapshotJson") AND
                       ...
                       (SELECT count(*) FROM __advance_schema__.quotation_technical_verifications tv
                         WHERE tv."VendorQuotationLineId"=ql."Id" AND tv."ComplianceStatus"='TechnicallyCompliant')=1;
                SELECT count(*) INTO actual_count FROM __advance_schema__.commercial_comparison_lines cl
                 WHERE cl."CommercialComparisonId"=NEW."Id" AND cl."IsRecommended";
                SELECT count(*) INTO duplicate_count FROM (SELECT cl."VendorQuotationLineId" FROM __advance_schema__.commercial_comparison_lines cl
                 WHERE cl."CommercialComparisonId"=NEW."Id" AND cl."IsRecommended"
                 GROUP BY cl."VendorQuotationLineId" HAVING count(*)<>1) duplicate_rows;
                missing_count:=greatest(expected_count-matched_count,0);
                SELECT count(*) INTO unexpected_count FROM __advance_schema__.commercial_comparison_lines cl
                 WHERE cl."CommercialComparisonId"=NEW."Id" AND cl."IsRecommended" AND NOT EXISTS
                       (SELECT 1 FROM __advance_schema__.vendor_quotation_lines ql WHERE ql."Id"=cl."VendorQuotationLineId" AND ql."VendorQuotationId"=NEW."RecommendedVendorQuotationId");
                SELECT count(*) INTO stale_version_count FROM __advance_schema__.commercial_comparison_lines cl
                 JOIN __advance_schema__.vendor_quotation_lines ql ON ql."Id"=cl."VendorQuotationLineId"
                 JOIN __advance_schema__.vendor_quotations q ON q."Id"=ql."VendorQuotationId"
                 WHERE cl."CommercialComparisonId"=NEW."Id" AND cl."IsRecommended" AND NOT q."IsCurrentRevision";
                SELECT count(*) INTO organization_mismatch_count FROM __advance_schema__.commercial_comparison_lines cl
                 JOIN __advance_schema__.vendor_quotation_lines ql ON ql."Id"=cl."VendorQuotationLineId"
                 JOIN __advance_schema__.vendor_quotations q ON q."Id"=ql."VendorQuotationId"
                 WHERE cl."CommercialComparisonId"=NEW."Id" AND cl."IsRecommended" AND q."OrganizationId" IS DISTINCT FROM NEW."OrganizationId";
                SELECT count(*) INTO parent_provenance_mismatch_count FROM __advance_schema__.commercial_comparison_lines cl
                 JOIN __advance_schema__.vendor_quotation_lines ql ON ql."Id"=cl."VendorQuotationLineId"
                 JOIN __advance_schema__.vendor_quotations q ON q."Id"=ql."VendorQuotationId"
                 JOIN __advance_schema__.rfq_vendor_invitations i ON i."Id"=q."RfqVendorInvitationId"
                 WHERE cl."CommercialComparisonId"=NEW."Id" AND cl."IsRecommended" AND
                ...
                SELECT count(*) INTO commercial_value_mismatch_count FROM __advance_schema__.commercial_comparison_lines cl
                 JOIN __advance_schema__.vendor_quotation_lines ql ON ql."Id"=cl."VendorQuotationLineId"
                 WHERE cl."CommercialComparisonId"=NEW."Id" AND cl."IsRecommended" AND
                       __advance_schema__.rev869b_commercial_snapshot_reconciles(ql."Id",cl."CommercialSnapshotJson",ql."TaxRuleSnapshotJson") IS NOT TRUE;
                SELECT count(*) INTO tax_mismatch_count FROM __advance_schema__.commercial_comparison_lines cl
                 JOIN __advance_schema__.vendor_quotation_lines ql ON ql."Id"=cl."VendorQuotationLineId"
                 WHERE cl."CommercialComparisonId"=NEW."Id" AND cl."IsRecommended" AND
                       (cl."CommercialSnapshotJson"->'taxRule' IS NULL OR cl."CommercialSnapshotJson"->'taxRule' IS DISTINCT FROM ql."TaxRuleSnapshotJson");
                SELECT count(*) INTO attachment_qualification_mismatch_count FROM __advance_schema__.commercial_comparison_lines cl
                 JOIN __advance_schema__.vendor_quotation_lines ql ON ql."Id"=cl."VendorQuotationLineId"
                 JOIN __advance_schema__.vendor_quotations q ON q."Id"=ql."VendorQuotationId"
                 JOIN __advance_schema__.rfq_vendor_invitations i ON i."Id"=q."RfqVendorInvitationId"
                 WHERE cl."CommercialComparisonId"=NEW."Id" AND cl."IsRecommended" AND
                ...
                SELECT count(*) INTO approval_mismatch_count FROM __advance_schema__.commercial_comparison_lines cl
                 WHERE cl."CommercialComparisonId"=NEW."Id" AND cl."IsRecommended" AND
                       (SELECT count(*) FROM __advance_schema__.quotation_technical_verifications tv
                ...
                IF expected_count=0 OR matched_count<>expected_count OR actual_count<>expected_count OR
                   ...
                   NEW."TotalPayableValue" IS DISTINCT FROM (SELECT sum(cl."TotalPayableValue") FROM __advance_schema__.commercial_comparison_lines cl WHERE cl."CommercialComparisonId"=NEW."Id" AND cl."IsRecommended")
            ...
            ELSIF TG_TABLE_NAME='purchase_orders' AND NEW."Status" IN ('PendingApproval','Resubmitted','Approved','Issued') THEN
                SELECT count(*) INTO expected_count FROM __advance_schema__.vendor_quotation_lines ql
                JOIN __advance_schema__.commercial_comparisons c ON c."RecommendedVendorQuotationId"=ql."VendorQuotationId"
                 WHERE c."Id"=NEW."CommercialComparisonId" AND c."Status"='Approved';
                SELECT count(*) INTO matched_count FROM __advance_schema__.purchase_order_lines pl
                JOIN __advance_schema__.commercial_comparison_lines cl ON cl."Id"=pl."CommercialComparisonLineId"
                JOIN __advance_schema__.vendor_quotation_lines ql ON ql."Id"=cl."VendorQuotationLineId"
                JOIN __advance_schema__.vendor_quotations q ON q."Id"=ql."VendorQuotationId"
                JOIN __advance_schema__.rfq_vendor_invitations i ON i."Id"=q."RfqVendorInvitationId"
                JOIN __advance_schema__.request_for_quotations r ON r."Id"=i."RequestForQuotationId"
                JOIN __advance_schema__.request_for_quotation_lines rl ON rl."Id"=ql."RequestForQuotationLineId"
                JOIN __advance_schema__.items item ON item."Id"=rl."ItemId"
                JOIN __advance_schema__.uoms uom ON uom."Id"=item."BaseUomId"
                JOIN __advance_schema__.vendors vendor ON vendor."Id"=q."VendorId"
                JOIN __advance_schema__.vendor_qualifications qualification ON qualification."VendorId"=vendor."Id"
                  AND qualification."OrganizationId"=q."OrganizationId"
                  AND qualification."ItemCategoryId"=item."CategoryId"
                JOIN __advance_schema__.commercial_comparisons c ON c."Id"=NEW."CommercialComparisonId"
                 WHERE pl."PurchaseOrderId"=NEW."Id" AND cl."CommercialComparisonId"=c."Id" AND cl."IsRecommended" AND
                       ...
                       __advance_schema__.rev869b_commercial_snapshot_reconciles(ql."Id",pl."CommercialSnapshotJson",pl."TaxRuleSnapshotJson") AND
                       ...
                       (SELECT count(*) FROM __advance_schema__.purchase_transaction_approval_history ah
                ...
                SELECT count(*) INTO actual_count FROM __advance_schema__.purchase_order_lines pl WHERE pl."PurchaseOrderId"=NEW."Id";
                SELECT count(*) INTO duplicate_count FROM (SELECT pl."CommercialComparisonLineId" FROM __advance_schema__.purchase_order_lines pl
                 WHERE pl."PurchaseOrderId"=NEW."Id" GROUP BY pl."CommercialComparisonLineId" HAVING count(*)<>1) duplicate_rows;
                missing_count:=greatest(expected_count-matched_count,0);
                SELECT count(*) INTO unexpected_count FROM __advance_schema__.purchase_order_lines pl
                ...
                SELECT count(*) INTO stale_version_count FROM __advance_schema__.purchase_order_lines pl
                 JOIN __advance_schema__.commercial_comparison_lines cl ON cl."Id"=pl."CommercialComparisonLineId"
                 JOIN __advance_schema__.vendor_quotation_lines ql ON ql."Id"=cl."VendorQuotationLineId"
                 JOIN __advance_schema__.vendor_quotations q ON q."Id"=ql."VendorQuotationId"
                ...
                SELECT count(*) INTO organization_mismatch_count FROM __advance_schema__.purchase_order_lines pl
                 WHERE pl."PurchaseOrderId"=NEW."Id" AND
                       (pl."CommercialSnapshotJson"->>'organizationId') IS DISTINCT FROM NEW."OrganizationId";
                SELECT count(*) INTO parent_provenance_mismatch_count FROM __advance_schema__.purchase_order_lines pl
                 JOIN __advance_schema__.commercial_comparison_lines cl ON cl."Id"=pl."CommercialComparisonLineId"
                 JOIN __advance_schema__.vendor_quotation_lines ql ON ql."Id"=cl."VendorQuotationLineId"
                 WHERE pl."PurchaseOrderId"=NEW."Id" AND
                ...
                SELECT count(*) INTO commercial_value_mismatch_count FROM __advance_schema__.purchase_order_lines pl
                 JOIN __advance_schema__.commercial_comparison_lines cl ON cl."Id"=pl."CommercialComparisonLineId"
                 JOIN __advance_schema__.vendor_quotation_lines ql ON ql."Id"=cl."VendorQuotationLineId"
                 WHERE pl."PurchaseOrderId"=NEW."Id" AND
                       __advance_schema__.rev869b_commercial_snapshot_reconciles(ql."Id",pl."CommercialSnapshotJson",pl."TaxRuleSnapshotJson") IS NOT TRUE;
                SELECT count(*) INTO tax_mismatch_count FROM __advance_schema__.purchase_order_lines pl
                 JOIN __advance_schema__.commercial_comparison_lines cl ON cl."Id"=pl."CommercialComparisonLineId"
                 JOIN __advance_schema__.vendor_quotation_lines ql ON ql."Id"=cl."VendorQuotationLineId"
                 WHERE pl."PurchaseOrderId"=NEW."Id" AND pl."TaxRuleSnapshotJson" IS DISTINCT FROM ql."TaxRuleSnapshotJson";
                SELECT count(*) INTO attachment_qualification_mismatch_count FROM __advance_schema__.purchase_order_lines pl
                 WHERE pl."PurchaseOrderId"=NEW."Id" AND
                ...
                SELECT count(*) INTO approval_mismatch_count FROM __advance_schema__.purchase_order_lines pl
                 WHERE pl."PurchaseOrderId"=NEW."Id" AND
                ...
                IF expected_count=0 OR matched_count<>expected_count OR actual_count<>expected_count OR
                   ...
                   (SELECT q."CurrencyCode" FROM __advance_schema__.commercial_comparisons c JOIN __advance_schema__.vendor_quotations q ON q."Id"=c."RecommendedVendorQuotationId" WHERE c."Id"=NEW."CommercialComparisonId") OR
                   NEW."TotalPayableValue" IS DISTINCT FROM (SELECT sum(pl."TotalPayableValue") FROM __advance_schema__.purchase_order_lines pl WHERE pl."PurchaseOrderId"=NEW."Id")
                THEN RAISE EXCEPTION 'PO exact source/version/cardinality/commercial provenance failed.'; END IF;

                SELECT count(*) INTO approval_count FROM __advance_schema__.purchase_transaction_approval_policies p
                ...
                IF NEW."Status"='Issued' AND (SELECT count(*) FROM __advance_schema__.purchase_order_history h
        ...
        CREATE OR REPLACE FUNCTION __advance_schema__.rev869b_guard_extended_immutability()
        RETURNS trigger LANGUAGE plpgsql SET search_path = pg_catalog, __advance_schema__ AS $rev869b$
        BEGIN
        ...
        CREATE TRIGGER trg_rev869b_rfq_line_insert_guard BEFORE INSERT ON __advance_schema__.request_for_quotation_lines FOR EACH ROW EXECUTE FUNCTION __advance_schema__.rev869b_guard_child_insert();
        CREATE TRIGGER trg_rev869b_invitation_insert_guard BEFORE INSERT ON __advance_schema__.rfq_vendor_invitations FOR EACH ROW EXECUTE FUNCTION __advance_schema__.rev869b_guard_child_insert();
        CREATE TRIGGER trg_rev869b_quotation_line_insert_guard BEFORE INSERT ON __advance_schema__.vendor_quotation_lines FOR EACH ROW EXECUTE FUNCTION __advance_schema__.rev869b_guard_child_insert();
        CREATE TRIGGER trg_rev869b_comparison_line_insert_guard BEFORE INSERT ON __advance_schema__.commercial_comparison_lines FOR EACH ROW EXECUTE FUNCTION __advance_schema__.rev869b_guard_child_insert();
        CREATE TRIGGER trg_rev869b_po_line_insert_guard BEFORE INSERT ON __advance_schema__.purchase_order_lines FOR EACH ROW EXECUTE FUNCTION __advance_schema__.rev869b_guard_child_insert();
        CREATE TRIGGER trg_rev869b_technical_insert_guard BEFORE INSERT ON __advance_schema__.quotation_technical_verifications FOR EACH ROW EXECUTE FUNCTION __advance_schema__.rev869b_guard_child_insert();
        CREATE TRIGGER trg_rev869b_followup_insert_guard BEFORE INSERT ON __advance_schema__.material_followup_handoffs FOR EACH ROW EXECUTE FUNCTION __advance_schema__.rev869b_guard_child_insert();
        CREATE TRIGGER trg_rev869b_comparison_authoritative_guard BEFORE UPDATE ON __advance_schema__.commercial_comparisons FOR EACH ROW EXECUTE FUNCTION __advance_schema__.rev869b_guard_authoritative_transition();
        CREATE TRIGGER trg_rev869b_po_authoritative_guard BEFORE UPDATE ON __advance_schema__.purchase_orders FOR EACH ROW EXECUTE FUNCTION __advance_schema__.rev869b_guard_authoritative_transition();
        CREATE TRIGGER trg_rev869b_comparison_history_insert_guard BEFORE INSERT ON __advance_schema__.purchase_transaction_approval_history FOR EACH ROW EXECUTE FUNCTION __advance_schema__.rev869b_guard_history_insert();
        CREATE TRIGGER trg_rev869b_po_history_insert_guard BEFORE INSERT ON __advance_schema__.purchase_order_history FOR EACH ROW EXECUTE FUNCTION __advance_schema__.rev869b_guard_history_insert();
        CREATE TRIGGER trg_rev869b_status_history_insert_guard BEFORE INSERT ON __advance_schema__.purchase_transaction_status_history FOR EACH ROW EXECUTE FUNCTION __advance_schema__.rev869b_guard_history_insert();
        CREATE TRIGGER trg_rev869b_rfq_lines_immutable BEFORE UPDATE OR DELETE ON __advance_schema__.request_for_quotation_lines FOR EACH ROW EXECUTE FUNCTION __advance_schema__.rev869b_guard_extended_immutability();
        CREATE TRIGGER trg_rev869b_invitation_snapshot_immutable BEFORE UPDATE OR DELETE ON __advance_schema__.rfq_vendor_invitations FOR EACH ROW EXECUTE FUNCTION __advance_schema__.rev869b_guard_extended_immutability();
        CREATE TRIGGER trg_rev869b_comparison_lines_delete_guard BEFORE DELETE ON __advance_schema__.commercial_comparison_lines FOR EACH ROW EXECUTE FUNCTION __advance_schema__.rev869b_guard_extended_immutability();
        CREATE TRIGGER trg_rev869b_followup_immutable BEFORE UPDATE OR DELETE ON __advance_schema__.material_followup_handoffs FOR EACH ROW EXECUTE FUNCTION __advance_schema__.rev869b_guard_extended_immutability();
        """;

    public static string Remove => AdvanceSchemaSql.Expand(RemoveTemplate);
    private const string RemoveTemplate = """
        DROP FUNCTION IF EXISTS __advance_schema__.rev869b_qualification_provenance_valid(uuid) CASCADE;
        DROP FUNCTION IF EXISTS __advance_schema__.rev869b_guard_history_insert() CASCADE;
        DROP FUNCTION IF EXISTS __advance_schema__.rev869b_guard_extended_immutability() CASCADE;
        DROP FUNCTION IF EXISTS __advance_schema__.rev869b_guard_authoritative_transition() CASCADE;
        DROP FUNCTION IF EXISTS __advance_schema__.rev869b_guard_child_insert() CASCADE;
        DROP FUNCTION IF EXISTS __advance_schema__.rev869b_commercial_snapshot_reconciles(uuid,jsonb,jsonb) CASCADE;
        """;
}
