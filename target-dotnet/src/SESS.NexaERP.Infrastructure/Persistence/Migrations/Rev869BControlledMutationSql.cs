...
internal static class Rev869BControlledMutationSql
{
    public const string Install = """
        ...
        BEGIN
          IF EXISTS (SELECT 1 FROM nexa.vendor_qualifications q WHERE NOT (
        ...
        CREATE OR REPLACE FUNCTION nexa.rev869b_guard_durable_audit_retention()
        RETURNS trigger LANGUAGE plpgsql SET search_path=pg_catalog,nexa AS $rev869b$
        DECLARE cleanup_reason text:=nullif(trim(current_setting('nexa.rev869b_audit_cleanup_reason',true)),'');
          cleanup_correlation text:=nullif(trim(current_setting('nexa.rev869b_audit_cleanup_correlation',true)),'');
          database_owner name;
        BEGIN
          IF TG_OP='UPDATE' THEN
            RAISE EXCEPTION USING ERRCODE='42501',SCHEMA='nexa',TABLE='audit_logs',
          ...
          RETURN OLD;
        END $rev869b$;
        REVOKE ALL ON FUNCTION nexa.rev869b_guard_durable_audit_retention() FROM PUBLIC;
        CREATE TRIGGER trg_rev869b_durable_audit_retention BEFORE UPDATE OR DELETE ON nexa.audit_logs
          FOR EACH ROW EXECUTE FUNCTION nexa.rev869b_guard_durable_audit_retention();
        ...
        BEGIN
          IF TG_TABLE_NAME='purchase_transaction_status_history' THEN
            SELECT count(*),min(p.version),min(p.login),min(p.org),min(p.number),min(p.status),min(p.correlation),min(p.creator)
              ...
              SELECT r."Version" version,coalesce(r."UpdatedBy",r."CreatedBy") login,r."OrganizationId" org,r."RfqNumber" number,r."Status" status,r."TransitionCorrelationId" correlation,r."CreatedBy" creator FROM nexa.request_for_quotations r WHERE NEW."EntityType"='RFQ' AND r."Id"=NEW."EntityId" AND r.xmin::text::bigint=txid_current()
              UNION ALL SELECT i."Version",coalesce(i."UpdatedBy",i."CreatedBy"),r."OrganizationId",r."RfqNumber",i."Status",i."TransitionCorrelationId",i."CreatedBy" FROM nexa.rfq_vendor_invitations i JOIN nexa.request_for_quotations r ON r."Id"=i."RequestForQuotationId" WHERE NEW."EntityType"='RFQInvitation' AND i."Id"=NEW."EntityId" AND i.xmin::text::bigint=txid_current()
              UNION ALL SELECT q."Version",coalesce(q."UpdatedBy",q."CreatedBy"),q."OrganizationId",q."QuotationNumber",q."Status",q."TransitionCorrelationId",q."CreatedBy" FROM nexa.vendor_quotations q WHERE NEW."EntityType"='VendorQuotation' AND q."Id"=NEW."EntityId" AND q.xmin::text::bigint=txid_current()
              UNION ALL SELECT v."Version",coalesce(v."UpdatedBy",v."CreatedBy"),q."OrganizationId",q."QuotationNumber",v."ComplianceStatus",v."CorrelationId",v."CreatedBy" FROM nexa.quotation_technical_verifications v JOIN nexa.vendor_quotation_lines l ON l."Id"=v."VendorQuotationLineId" JOIN nexa.vendor_quotations q ON q."Id"=l."VendorQuotationId" WHERE NEW."EntityType"='TechnicalVerification' AND v."Id"=NEW."EntityId" AND v.xmin::text::bigint=txid_current()
              UNION ALL SELECT c."Version",coalesce(c."UpdatedBy",c."CreatedBy"),c."OrganizationId",c."ComparisonNumber",c."Status",c."TransitionCorrelationId",c."CreatedBy" FROM nexa.commercial_comparisons c WHERE NEW."EntityType"='CommercialComparison' AND c."Id"=NEW."EntityId" AND c.xmin::text::bigint=txid_current()
              UNION ALL SELECT p."Version",coalesce(p."UpdatedBy",p."CreatedBy"),p."OrganizationId",p."PoNumber",p."Status",p."TransitionCorrelationId",p."CreatedBy" FROM nexa.purchase_orders p WHERE NEW."EntityType"='PurchaseOrder' AND p."Id"=NEW."EntityId" AND p.xmin::text::bigint=txid_current()
              UNION ALL SELECT h."Version",coalesce(h."UpdatedBy",h."CreatedBy"),p."OrganizationId",h."HandoffNumber",h."Status",h."CorrelationId",h."CreatedBy" FROM nexa.material_followup_handoffs h JOIN nexa.purchase_orders p ON p."Id"=h."PurchaseOrderId" WHERE NEW."EntityType"='MaterialFollowUp' AND h."Id"=NEW."EntityId" AND h.xmin::text::bigint=txid_current()
            ) p;
          ...
          IF NEW."Action"='Approve' AND TG_TABLE_NAME='purchase_transaction_approval_history' AND EXISTS (
            SELECT 1 FROM nexa.commercial_comparisons c JOIN nexa.commercial_comparison_lines cl ON cl."CommercialComparisonId"=c."Id" AND cl."IsRecommended"
            JOIN nexa.vendor_quotation_lines ql ON ql."Id"=cl."VendorQuotationLineId" JOIN nexa.quotation_technical_verifications v ON v."VendorQuotationLineId"=ql."Id"
          ...
          IF NEW."Action"='Approve' AND TG_TABLE_NAME='purchase_order_history' AND EXISTS (
            SELECT 1 FROM nexa.purchase_transaction_status_history h WHERE h."EntityType"='PurchaseOrder' AND h."EntityId"=NEW."PurchaseOrderId"
          ...
          IF NEW."Action"='Issue' AND TG_TABLE_NAME='purchase_order_history' AND EXISTS (
            SELECT 1 FROM nexa.purchase_order_history approved
        ...
        BEGIN
          ...
          expected_employee:=nullif(current_setting('nexa.rev869b_actor_employee_id',true),'')::uuid;
        ...
        BEGIN
          IF current_setting('nexa.rev869b_actor_employee_id',true) IS NULL OR
             ...
             current_setting('nexa.rev869b_organization',true) IS NULL OR
        ...
        BEGIN
          IF TG_TABLE_NAME='quotation_technical_verifications' THEN
            ...
            IF NOT EXISTS (SELECT 1 FROM nexa.purchase_transaction_status_history h WHERE h."EntityType"='TechnicalVerification' AND h."EntityId"=NEW."Id"
        ...
        CREATE CONSTRAINT TRIGGER trg_rev869b_bound_qualification_history AFTER INSERT OR UPDATE ON nexa.vendor_qualifications DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION nexa.rev869b_require_qualification_history();
        ...
        CREATE CONSTRAINT TRIGGER trg_rev869b_bound_technical_history AFTER INSERT ON nexa.quotation_technical_verifications DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION nexa.rev869b_require_bound_history();
    ...
    public const string Remove = """
        DROP TRIGGER IF EXISTS trg_rev869b_durable_audit_retention ON nexa.audit_logs;
        DROP FUNCTION IF EXISTS nexa.rev869b_guard_durable_audit_retention();
        ALTER TABLE nexa.vendor_qualifications DROP CONSTRAINT IF EXISTS "CK_vendor_qualification_rev869b_lifecycle";
        DROP TRIGGER IF EXISTS trg_rev869b_qualification_history_insert_guard ON nexa.controlled_configuration_histories;
