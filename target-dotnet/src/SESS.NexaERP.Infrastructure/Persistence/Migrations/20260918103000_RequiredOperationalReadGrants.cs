...
[Migration("20260918103000_RequiredOperationalReadGrants")]
public sealed class RequiredOperationalReadGrants : Migration
{
        ...
        migrationBuilder.Sql("""
            ...
            BEGIN
              FOR required IN SELECT * FROM (VALUES
                ('PURCHASE_EXECUTIVE','masters.vendors',false),
                ('PURCHASE_MANAGER','masters.vendors',false),
                ('TECHNICAL_DIRECTOR','production.job-orders',false)) v(role_code,page_key,download_required)
              LOOP
                ...
                IF prior IS NULL THEN
                  grant_id := md5('required-operational-read:'||required.role_code||':'||required.page_key)::uuid;
                  ...
                  VALUES(grant_id,role_id,page_id,
                    ...
                    false,false,false,false,clock_timestamp(),'RequiredOperationalReadGrants',0);
                ELSIF NOT ((prior->>'CanView')::boolean OR (required.page_key<>'purchase.vendor-quotations' AND (prior->>'HasFullControl')::boolean))
                  ...
                  UPDATE advance.role_page_permissions SET "CanView"=true,"CanDownload"="CanDownload" OR required.download_required,"Version"="Version"+1,
                    "UpdatedAt"=clock_timestamp(),"UpdatedBy"='RequiredOperationalReadGrants' WHERE "Id"=grant_id;
                END IF;
                ...
                VALUES(gen_random_uuid(),'GLOBAL','Security','GrantRequiredRead','RolePagePermission',grant_id::text,
                  session_user,'','Success','RequiredOperationalReadGrants:'||required.role_code||':'||required.page_key,
                  prior::text,after_row::text,clock_timestamp(),'RequiredOperationalReadGrants',0);
              END LOOP;
            END $grant$;
        ...
        migrationBuilder.Sql("""
            ...
            BEGIN
              IF (SELECT count(*) FROM advance.audit_logs a
                WHERE a."CreatedBy"='RequiredOperationalReadGrants' AND a."Action"='GrantRequiredRead'
                  AND NOT EXISTS(SELECT 1 FROM advance.audit_logs r WHERE r."CreatedBy"='RequiredOperationalReadGrants'
              ...
              LOOP
                ...
                VALUES(gen_random_uuid(),'GLOBAL','Security','RevertRequiredRead','RolePagePermission',grant_id::text,
                  ...
                  clock_timestamp(),'RequiredOperationalReadGrants',0);
              END LOOP;
            END $revert$;
            """.Replace("\r\n", "\n", StringComparison.Ordinal));
    }
