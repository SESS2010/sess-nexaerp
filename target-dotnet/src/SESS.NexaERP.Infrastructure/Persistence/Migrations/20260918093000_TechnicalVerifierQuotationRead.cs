...
[Migration("20260918093000_TechnicalVerifierQuotationRead")]
public sealed class TechnicalVerifierQuotationRead : Migration
{
        ...
        migrationBuilder.Sql("""
            ...
            BEGIN
              FOR required IN SELECT * FROM (VALUES
                ('TECHNICAL_SUPPORT_MANAGER','purchase.vendor-quotations',true)) v(role_code,page_key,download_required)
              LOOP
                ...
                IF prior IS NULL THEN
                  grant_id := md5('technical-verifier-read:'||required.role_code||':'||required.page_key)::uuid;
                  ...
                  VALUES(grant_id,role_id,page_id,
                    ...
                    false,false,false,false,clock_timestamp(),'TechnicalVerifierQuotationRead',0);
                ELSIF NOT ((prior->>'CanView')::boolean OR (required.page_key<>'purchase.vendor-quotations' AND (prior->>'HasFullControl')::boolean))
                  ...
                  UPDATE advance.role_page_permissions SET "CanView"=true,"CanDownload"="CanDownload" OR required.download_required,"Version"="Version"+1,
                    "UpdatedAt"=clock_timestamp(),"UpdatedBy"='TechnicalVerifierQuotationRead' WHERE "Id"=grant_id;
                END IF;
                ...
                VALUES(gen_random_uuid(),'GLOBAL','Security','GrantRequiredRead','RolePagePermission',grant_id::text,
                  session_user,'','Success','TechnicalVerifierQuotationRead:'||required.role_code||':'||required.page_key,
                  prior::text,after_row::text,clock_timestamp(),'TechnicalVerifierQuotationRead',0);
              END LOOP;
            END $grant$;
        ...
        migrationBuilder.Sql("""
            ...
            BEGIN
              IF (SELECT count(*) FROM advance.audit_logs a
                WHERE a."CreatedBy"='TechnicalVerifierQuotationRead' AND a."Action"='GrantRequiredRead'
                  AND NOT EXISTS(SELECT 1 FROM advance.audit_logs r WHERE r."CreatedBy"='TechnicalVerifierQuotationRead'
                    AND r."Action"='RevertRequiredRead' AND r."CorrelationId"=a."Id"::text)) <> 1 THEN
                RAISE EXCEPTION 'Required read-grant rollback requires one intact unreverted receipt';
              END IF;
              ...
              LOOP
                ...
                VALUES(gen_random_uuid(),'GLOBAL','Security','RevertRequiredRead','RolePagePermission',grant_id::text,
                  ...
                  clock_timestamp(),'TechnicalVerifierQuotationRead',0);
              END LOOP;
            END $revert$;
            """.Replace("\r\n", "\n", StringComparison.Ordinal));
    }
