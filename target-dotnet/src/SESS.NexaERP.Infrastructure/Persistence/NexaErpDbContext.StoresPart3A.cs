...
public sealed partial class NexaErpDbContext
{
    ...
    private static void ConfigureMaterialIssues(ModelBuilder m)
    {
        m.Entity<MaterialIssueRequest>(e => {
            ...
            e.Property(x => x.RequestNumber).HasMaxLength(50).IsRequired(); e.Property(x => x.Purpose).HasMaxLength(30).IsRequired(); e.Property(x => x.Situation).HasMaxLength(40).IsRequired(); e.Property(x => x.DestinationType).HasMaxLength(20).IsRequired(); e.Property(x => x.DestinationNameSnapshot).HasMaxLength(240).IsRequired(); e.Property(x => x.Status).HasMaxLength(30).IsRequired();
        ...
        m.Entity<MaterialIssueRequestLine>(e => {
            ...
            e.Property(x => x.ItemCodeSnapshot).HasMaxLength(80).IsRequired(); e.Property(x => x.ItemNameSnapshot).HasMaxLength(240).IsRequired(); e.Property(x => x.UomSnapshot).HasMaxLength(32).IsRequired(); e.Property(x => x.RequestedQuantity).HasPrecision(24, 6); e.Property(x => x.RequestedBaseQuantity).HasPrecision(24, 6); e.Property(x => x.EstimatedBomBaseQuantitySnapshot).HasPrecision(24, 6); e.Property(x => x.ProductionBomBaseQuantitySnapshot).HasPrecision(24, 6); e.Property(x => x.CustomerPoBaseQuantitySnapshot).HasPrecision(24, 6); e.Property(x => x.ExcessBaseQuantitySnapshot).HasPrecision(24, 6); e.Property(x => x.ExcessClassification).HasMaxLength(30).IsRequired(); e.Property(x => x.Remarks).HasMaxLength(1000);
            e.HasOne(x => x.MaterialIssueRequest).WithMany(x => x.Lines).HasForeignKey(x => new { x.CompanyId, x.MaterialIssueRequestId }).HasPrincipalKey(x => new { x.CompanyId, x.Id }).OnDelete(DeleteBehavior.Restrict); e.HasOne(x => x.Item).WithMany().HasForeignKey(x => x.ItemId).OnDelete(DeleteBehavior.Restrict); e.HasOne(x => x.Uom).WithMany().HasForeignKey(x => x.UomId).OnDelete(DeleteBehavior.Restrict); e.HasOne(x => x.CustomerPurchaseOrderLine).WithMany().HasForeignKey(x => x.CustomerPurchaseOrderLineId).OnDelete(DeleteBehavior.Restrict);
        });
    }
