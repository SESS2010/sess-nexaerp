...
public sealed partial class NexaErpDbContext
{
    ...
    private static void ConfigureQc(ModelBuilder m)
    {
        m.Entity<QcInspection>(e => {
            ...
            e.HasIndex(x => new { x.CompanyId, x.InspectionNumber }).IsUnique(); e.HasIndex(x => x.GoodsReceiptLineId).IsUnique(); e.HasIndex(x => x.DeliveryChallanLineId).IsUnique(); e.Property(x => x.InspectionNumber).HasMaxLength(50).IsRequired();
        ...
        m.Entity<QcInspectionRevision>(e => {
            ...
            foreach(var n in new[]{nameof(QcInspectionRevision.InspectedQuantity),nameof(QcInspectionRevision.AcceptedQuantity),nameof(QcInspectionRevision.RejectedQuantity),nameof(QcInspectionRevision.InspectionShortfallRejectedQuantity)}) e.Property<decimal>(n).HasPrecision(24,6);
