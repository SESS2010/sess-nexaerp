...
public sealed class EfSessionService(NexaErpDbContext db, ICurrentUser currentUser, IPagePermissionService pagePermissions) : ISessionService
{
    public async Task<SessionMe> GetCurrentAsync(CancellationToken cancellationToken)
    {
        ...
        var permissions = await pagePermissions.ResolveEffectivePermissionsAsync(
            currentUser.EffectiveRoleAssignments, company.Code, employee.Id, cancellationToken);
