...
public sealed partial class EfRev869BPurchaseService
{
    ...
    public async Task<Rev869BDocumentResult> RecommendAsync(string number, Rev869BRecommendComparisonRequest request, CancellationToken ct)
    {
        ...
        {
            if (replay.Action != "Recommend" || replay.Remarks != request.RecommendationRemarks.Trim()) throw new Rev869BConflictException("Recommendation idempotency key was reused with a different command.");
        ...
        {
            var recalculated = await RecalculateAsync(quoteLine, organization, DateOnly.FromDateTime(DateTime.UtcNow), ct); total += recalculated.Breakdown.TotalPayableValue;
    ...
    public async Task<Rev869BDocumentResult> CreatePurchaseOrderAsync(Rev869BCreatePurchaseOrderRequest request, CancellationToken ct)
    {
        ...
        foreach (var ql in quote.Lines)
        {
            ...
            po.TaxableValue += calc.TaxableValue; po.DiscountValue += calc.DiscountValue; po.TaxValue += calc.CgstValue + calc.SgstValue + calc.IgstValue + calc.CessValue; po.PackingForwarding += calc.PackingForwarding; po.Freight += calc.Freight; po.Insurance += calc.Insurance; po.OtherCharges += calc.OtherCharges; po.RoundOff += calc.RoundOff; po.TotalPayableValue += calc.TotalPayableValue;
        }
    ...
    public async Task<Rev869BDocumentResult> SubmitPurchaseOrderAsync(string number, Rev869BSubmitPurchaseOrderRequest request, CancellationToken ct)
    {
        ...
        var po = await db.PurchaseOrders.AsNoTracking().Include(x => x.Lines).Where(x => x.OrganizationId == organization && x.PoNumber == number.Trim().ToUpper() && x.Status == Rev869BStatuses.Draft).OrderByDescending(x => x.RevisionNumber).FirstOrDefaultAsync(ct)
            ?? throw new Rev869BNotFoundException("Draft purchase-order version was not found.");
        ...
        if (replay is not null) { if (replay.Action != "Submit" || replay.Reason != request.Remarks.Trim()) throw new Rev869BConflictException("PO submit idempotency key was reused."); await tx.RollbackAsync(ct); return Result(po.Id, po.PoNumber, po.Status, po.Version); }
        var version = await ReservePoAsync(po.Id, organization, request.Version, ct); var remarks = RequiredRemarks(request.Remarks); Rev869BStatusContracts.RequirePurchaseOrder(po.Status, Rev869BStatuses.PendingApproval);
        await db.PurchaseOrders.Where(x => x.Id == po.Id && x.OrganizationId == organization && x.Version == version).ExecuteUpdateAsync(s => s.SetProperty(x => x.Status, Rev869BStatuses.PendingApproval).SetProperty(x => x.UpdatedAt, DateTimeOffset.UtcNow).SetProperty(x => x.UpdatedBy, user.LoginId), ct);
        AddPoHistory(po, "Submit", po.Status, Rev869BStatuses.PendingApproval, remarks, request.IdempotencyKey); AddStatus("PurchaseOrder", po.Id, po.PoNumber, po.Status, Rev869BStatuses.PendingApproval, "Submit", remarks, request.IdempotencyKey);
        await db.SaveChangesAsync(ct); await audit.WriteAsync("Purchase", "SubmitPO", nameof(PurchaseOrder), po.Id.ToString(), new { status = po.Status }, new { status = Rev869BStatuses.PendingApproval, po.ApprovalRoute, po.TotalPayableValue }, ct); await tx.CommitAsync(ct); return Result(po.Id, po.PoNumber, Rev869BStatuses.PendingApproval, version);
    }
    ...
    {
        var actor = RequireActor(); RequireRole(Rev869ARoleCodes.PurchaseManager); await using var tx = await db.Database.BeginTransactionAsync(IsolationLevel.Serializable, ct); var prior = await LoadPoAsync(number, ct); await AuthorizePoAsync(actor, prior, ct);
        var replay = await db.PurchaseOrders.AsNoTracking().SingleOrDefaultAsync(x => x.OrganizationId == prior.OrganizationId && x.IdempotencyKey == request.IdempotencyKey.Trim(), ct);
        if (replay is not null) { if (replay.PreviousVersionId != prior.Id || replay.AmendmentReason != request.AmendmentReason.Trim()) throw new Rev869BConflictException("PO amendment idempotency key was reused."); await tx.RollbackAsync(ct); return Result(replay.Id, replay.PoNumber, replay.Status, replay.Version); }
    ...
    private async Task<Rev869BDocumentResult> PurchaseOrderApprovalActionAsync(string number, Rev869BPoApprovalActionRequest request, bool approve, CancellationToken ct)
    {
        ...
        var po = await db.PurchaseOrders.AsNoTracking().Include(x => x.Lines).Where(x => x.OrganizationId == organization && x.PoNumber == number.Trim().ToUpper() && x.Status == Rev869BStatuses.PendingApproval).OrderByDescending(x => x.RevisionNumber).FirstOrDefaultAsync(ct)
            ?? throw new Rev869BNotFoundException("Pending-approval purchase-order version was not found.");
        ...
        var replay = await db.PurchaseOrderHistories.AsNoTracking().SingleOrDefaultAsync(x => x.PurchaseOrderId == po.Id && x.CorrelationId == request.IdempotencyKey.Trim(), ct);
        if (replay is not null) { if (replay.Action != action || replay.Reason != request.Remarks.Trim()) throw new Rev869BConflictException("PO approval idempotency key was reused."); await tx.RollbackAsync(ct); return Result(po.Id, po.PoNumber, po.Status, po.Version); }
        var calculatedTotal = po.Lines.Sum(x => x.TotalPayableValue); if (calculatedTotal != po.TotalPayableValue) throw new Rev869BConflictException("PO header/line payable values are inconsistent.");
        ...
        {
            prior = await db.PurchaseOrders.AsNoTracking().SingleOrDefaultAsync(x => x.Id == po.PreviousVersionId && x.OrganizationId == organization && x.IsCurrentVersion && x.Status == Rev869BStatuses.Issued, ct) ?? throw new Rev869BConflictException("Issued current predecessor is unavailable.");
        ...
        await db.PurchaseOrders.Where(x => x.Id == po.Id && x.OrganizationId == organization && x.Version == version).ExecuteUpdateAsync(s => s.SetProperty(x => x.Status, next).SetProperty(x => x.IsCurrentVersion, approve || po.IsCurrentVersion).SetProperty(x => x.UpdatedAt, DateTimeOffset.UtcNow).SetProperty(x => x.UpdatedBy, user.LoginId), ct);
        ...
        await db.SaveChangesAsync(ct); await audit.WriteAsync("Purchase", action + "PO", nameof(PurchaseOrder), po.Id.ToString(), new { status = po.Status, po.IsCurrentVersion }, new { status = next, IsCurrentVersion = approve || po.IsCurrentVersion, route, calculatedTotal }, ct); await tx.CommitAsync(ct); return Result(po.Id, po.PoNumber, next, version);
    }
