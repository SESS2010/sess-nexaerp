...
public sealed class EfPurchaseObligationsService(
    NexaErpDbContext db, ICurrentUser user, IOptions<ReportCalendarOptions> calendar)
    : IPurchaseObligationsService
{
    private static readonly JsonSerializerOptions Json = new(JsonSerializerDefaults.Web);
    public async Task<PurchaseObligationsPage> GetAsync(PurchaseObligationsRequest request, CancellationToken ct)
    {
        ...
        if (request.Page < 1 || request.PageSize is < 1 or > 1000 ||
            request.Queue is not (null or "grni" or "vendor-advances"))
            throw new ReportRequestException("Choose GRNI or vendor advances and a page size from 1 to 1000.");
        ...
        await using var command = new NpgsqlCommand(
            "SELECT advance.purchase_obligations(@organization,@employee,@assignments,@report_timezone," +
            "@queue,@vendor,@currency,@document,@offset,@page_size)",
        ...
        if (!result.RootElement.GetProperty("requestValid").GetBoolean())
            throw new ReportRequestException("Choose a supported queue and page.");
        if (!result.RootElement.GetProperty("valid").GetBoolean()) throw new PurchaseObligationSourceException();
        ...
        return new(organization, payload.GetProperty("generatedAt").GetDateTimeOffset(), zone.Id,
            payload.GetProperty("tiles").Deserialize<PurchaseObligationTile[]>(Json)!,
            payload.GetProperty("vendors").Deserialize<PurchaseObligationVendor[]>(Json)!, request,
            payload.GetProperty("totalRows").GetInt64(),
            payload.GetProperty("rows").Deserialize<PurchaseObligationRow[]>(Json)!);
    }
}
