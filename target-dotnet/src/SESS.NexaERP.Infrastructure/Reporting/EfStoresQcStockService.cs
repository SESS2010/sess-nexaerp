...
public sealed class EfStoresQcStockService(NexaErpDbContext db,ICurrentUser user,IOptions<ReportCalendarOptions> calendar)
    : IStoresQcStockService
{
    private static readonly JsonSerializerOptions Json=new(JsonSerializerDefaults.Web);
    public async Task<StoresQcStockPage> GetAsync(StoresQcStockRequest request,CancellationToken ct)
    {
        ...
        if(request.Queue is not(null or "QC_HOLD" or "PENDING_RETURNABLE_DC"))
            throw new ReportRequestException("Unknown Stores QC stock queue.");
        ...
        await using var command=new NpgsqlCommand(
            "SELECT advance.stores_qc_stock(@organization,@employee,@assignments,@report_timezone,@queue,@document,@offset,@page_size)",
        ...
        return new(organization,data.GetProperty("generatedAt").GetDateTimeOffset(),zone.Id,
            data.GetProperty("commercial").GetBoolean(),data.GetProperty("valueBasis").GetString()!,
            data.GetProperty("tiles").Deserialize<StoresQcStockTile[]>(Json)!,request,
            data.GetProperty("totalRows").GetInt64(),data.GetProperty("rows").Deserialize<StoresQcStockRow[]>(Json)!);
    }
}
