...
public static class StoresGateEntryEndpoints
{
    ...
    public static IEndpointRouteBuilder MapStoresGateEntryEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        g.MapGet("/",async(string? gateEntryNumber,string? purchaseOrderNumber,Guid? vendorId,DateOnly? from,DateOnly? to,string? state,string? sortBy,string? sortDirection,int? page,int? pageSize,IGateEntryService s,HttpContext h,CancellationToken ct)=>await Run(()=>s.ListAsync(gateEntryNumber,purchaseOrderNumber,vendorId,from,to,state,sortBy,sortDirection,page??1,pageSize??50,ct),h)).RequirePagePermission(Page,PagePermissionActions.View);
        return endpoints;
    }
