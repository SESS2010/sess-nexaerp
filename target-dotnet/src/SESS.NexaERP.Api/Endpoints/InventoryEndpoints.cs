...
public static class InventoryEndpoints
{
    public static IEndpointRouteBuilder MapInventoryEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapGet("/items", async (NexaErpDbContext db, int? page, int? pageSize, string? search, string? status, string? category, string? sortBy, string? sortDirection, CancellationToken ct) =>
        {
            ...
            var rows = await q.Skip(p.Skip).Take(p.PageSize).Select(x => new ItemSummary(x.Id, x.ItemCode, x.Name, x.Uom, x.MaterialType, x.ManufacturerMake, x.Model, x.PartNumber, x.MinimumStock, x.MaximumStock, x.ReorderLevel, x.Status, x.ApprovalStatus, x.IsActive, x.Version)).ToListAsync(ct);
        ...
        group.MapPost("/items", async (UpsertItemRequest r, NexaErpDbContext db, IAuditWriter audit, ICurrentUser user, CancellationToken ct) =>
        {
            ...
            var item = new Item(); Apply(item, r, user.LoginId, true); db.Items.Add(item); AddInitialStatus(db, nameof(Item), item.Id, item.ItemCode, item.Status, user.LoginId);
        ...
        group.MapPut("/items/{code}", async (string code, UpsertItemRequest r, NexaErpDbContext db, IAuditWriter audit, ICurrentUser user, CancellationToken ct) =>
        {
            ...
            var validation = await ValidateItem(r, db, item.Id, ct); if (validation is not null) return validation; var before = ToDetail(item); Apply(item, r, user.LoginId, false);
    ...
    private static async Task<IResult?> ValidateItem(UpsertItemRequest r, NexaErpDbContext db, Guid? id, CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(code) || string.IsNullOrWhiteSpace(r.Name) || string.IsNullOrWhiteSpace(r.Uom) || string.IsNullOrWhiteSpace(r.DetailedDescription) || string.IsNullOrWhiteSpace(r.MaterialType)) return Results.BadRequest(new { message = "Item code, name, description, material type and UOM are required." });
        ...
        if (r.GstPercentage < 0 || r.GstPercentage > 28) return Results.BadRequest(new { message = "GST percentage must be in configured valid range." });
