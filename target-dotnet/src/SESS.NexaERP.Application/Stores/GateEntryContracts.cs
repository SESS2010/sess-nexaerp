...
public sealed record GateEntryListResult(int Page,int PageSize,IReadOnlyList<GateEntryResult> Items);
...
public interface IGateEntryService
{
    ...
    Task<GateEntryListResult> ListAsync(string? purchaseOrderNumber,Guid? vendorId,DateOnly? from,DateOnly? to,string? state,int page,int pageSize,CancellationToken cancellationToken);
}
