...
internal static class IntercompanyRoutes20260915AccessSql
{
    internal const string Provision = """
        DO $ic_acl$ DECLARE relation text; signature text; principal text;
        BEGIN
         IF to_regclass('advance.intercompany_routes') IS NOT NULL THEN
          FOREACH relation IN ARRAY ARRAY['intercompany_routes','intercompany_route_decisions'] LOOP
          ...
          FOREACH signature IN ARRAY ARRAY[
           'advance.guard_intercompany_route_evidence()','advance.validate_intercompany_route(advance.intercompany_routes)',
           'advance.intercompany_route_json(uuid,uuid)','advance.intercompany_routes_page(uuid,integer,integer)',
           'advance.intercompany_route_options(uuid)','advance.record_intercompany_route(uuid,uuid,uuid,text,jsonb,uuid,text,uuid,text,text)'] LOOP
          ...
          IF to_regrole('nexa_erp_runtime') IS NOT NULL THEN
           GRANT EXECUTE ON FUNCTION advance.intercompany_route_json(uuid,uuid),advance.intercompany_routes_page(uuid,integer,integer),
            advance.intercompany_route_options(uuid),advance.record_intercompany_route(uuid,uuid,uuid,text,jsonb,uuid,text,uuid,text,text) TO nexa_erp_runtime;
          END IF;
         END IF;
        END $ic_acl$;
        """;
        ...
        BEGIN
         IF to_regclass('advance.intercompany_routes') IS NOT NULL THEN
          FOREACH relation IN ARRAY ARRAY['intercompany_routes','intercompany_route_decisions'] LOOP
           ...
           THEN RAISE EXCEPTION 'Intercompany route table owner or direct ACL is invalid: %',relation; END IF;
          END LOOP;
          FOREACH signature IN ARRAY ARRAY[
           'advance.guard_intercompany_route_evidence()','advance.validate_intercompany_route(advance.intercompany_routes)',
           'advance.intercompany_route_json(uuid,uuid)','advance.intercompany_routes_page(uuid,integer,integer)',
           ...
           is_public_entry:=signature NOT IN('advance.guard_intercompany_route_evidence()','advance.validate_intercompany_route(advance.intercompany_routes)');
           ...
           THEN RAISE EXCEPTION 'Intercompany route function owner, search path or ACL is invalid: %',signature; END IF;
          END LOOP;
         ELSIF to_regclass('advance.intercompany_route_decisions') IS NOT NULL
          OR to_regprocedure('advance.intercompany_route_options(uuid)') IS NOT NULL THEN
          RAISE EXCEPTION 'Intercompany route package is partially installed.';
         END IF;
        END $ic_verify$;
        """;
}
