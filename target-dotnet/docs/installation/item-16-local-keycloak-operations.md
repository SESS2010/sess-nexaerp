...
Keycloak differs from Cognito: flows can enforce conditional MFA inside a realm, including by role; two realms are the chosen design, not a Keycloak limitation. TECHNICAL_DIRECTOR, MANAGING_DIRECTOR and ACCOUNTS_MANAGER must use Approvers. ERP refuses all access in the affected company with MFA_REQUIRED if their active mapping resolves through Staff. Promotion requires creating the Approvers account, verifying its signed-token subject, revoking the old ERP mapping and creating the new mapping; never rewrite identity history.
