...
function New-Sha256Fingerprint([string]$Value) {
    $sha = $null
    try {
...
function Get-SanitizedEfFailure([object[]]$Output, [int]$ExitCode, [string]$Phase) {
    try {
        ...
        return "exit_code=$ExitCode; phase=$Phase; exception_type=sanitizer_failure; sqlstate=unknown; schema=unknown; table=unknown; column=unknown; constraint=unknown; message_category=sanitizer_failure; category=ef_migration_failure; raw_output_sha256=unavailable"
    }
}
