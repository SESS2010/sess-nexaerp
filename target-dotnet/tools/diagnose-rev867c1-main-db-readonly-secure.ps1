...
$reportFile = Join-Path $reportDir "rev867c1_main_db_readonly_diagnostic_$timestamp.md"
$securePassword = $null
$plainPassword = $null
$evidence = [ordered]@{}
$diagnosticSql = $null
...
function Write-SqlOnlyReport([System.Collections.Specialized.OrderedDictionary]$SqlMap) {
    ...
    Add-Report "# REV867C1 Main Development Database Read-Only SQL Source Verification"
    Add-Report ""
    ...
    Write-Host "REV867C1 main-db generated diagnostic SQL report: $reportFile"
}
...
function Invoke-PsqlRead([string]$Sql) {
    $sqlFile = Join-Path ([System.IO.Path]::GetTempPath()) ("sess_nexa_rev867c1_main_diag_" + [Guid]::NewGuid().ToString("N") + ".sql")
    try {
...
function Write-FailureReport([string]$Message) {
    New-Item -ItemType Directory -Force -Path $reportDir | Out-Null
    Add-Report "# REV867C1 Main Development Database Read-Only Diagnostic Failed"
    Add-Report ""
    Add-Report "- Time: $(Get-Date -Format o)"
    ...
    Write-Host "REV867C1 main-db read-only diagnostic failed. Sanitized report: $reportFile"
}

try {
    Write-Section "REV867C1 main-db read-only diagnostic prechecks"
    Assert-SafePgIdentifier $Database "Main development database name"
    Assert-SafePgIdentifier $UserName "PostgreSQL user name"
    if ($Database -ne "sess_nexaerp" -or $HostName -ne "localhost" -or $Port -ne 5432) { throw "This diagnostic is restricted to sess_nexaerp on localhost:5432." }
    ...
    if ($gitStatus) { throw "Git status is not clean before REV867C1 main-db read-only diagnostic." }

    Write-Section "REV867C1 main-db read-only PostgreSQL diagnostic"
    $securePassword = Read-Host -AsSecureString "Enter PostgreSQL password for local main development database only"
    ...
    if ($evidence["Session identity"] -notmatch "database=$Database") { throw "Connected database did not match expected main development database." }
    ...
    if ($evidence["Public EF history exists"] -match "not_found") {
        $evidence["Migration IDs"] = "skipped: public.__EFMigrationsHistory not found in this connected database"
        $evidence["REV867C1 migration present"] = "skipped: public.__EFMigrationsHistory not found in this connected database"
    } else {
        Add-Evidence "Migration IDs" $diagnosticSql["Migration IDs"]
        Add-Evidence "REV867C1 migration present" $diagnosticSql["REV867C1 migration present"]
    }

    Add-Report "# REV867C1 Main Development Database Read-Only Diagnostic Report"
    Add-Report ""
    ...
    Write-Host "REV867C1 main-db read-only diagnostic report: $reportFile"
}
