...
CREATE FUNCTION nexa.rev869b_read_lifecycle_facts_v4(instance_sha256 text,lease_id uuid,lease_version bigint,attempt_id uuid,request_id uuid,decision_id uuid,scenario_execution_id uuid,observation_stage text,subcase_id text,requested_facts text[]) RETURNS jsonb LANGUAGE sql SECURITY DEFINER VOLATILE SET search_path=pg_catalog,nexa AS $$
 WITH selected_lease AS (
  ...
  SELECT a.*,u.ordinality FROM unnest($10) WITH ORDINALITY u(name,ordinality) JOIN allowed a USING(name)
 ), valued AS (
  SELECT r.name,r.value_type,r.ordinality,
   CASE
    WHEN r.name='consumedAttemptId' THEN to_jsonb((SELECT d.ConsumedAttemptId FROM selected_decisions d)) WHEN r.name IN ('reconciledAttemptId','attemptId','survivingAttemptId') THEN to_jsonb((SELECT a.AttemptId FROM selected_attempts a))
    WHEN r.name='authorizedAction' THEN to_jsonb((SELECT d.AuthorizedAction FROM selected_decisions d)) WHEN r.name='performedAction' THEN to_jsonb((SELECT a.Kind FROM selected_attempts a))
    ...
    ELSE NULL::jsonb END value
  FROM requested r
 ), fact_rows AS (
  SELECT jsonb_build_object('kind',CASE WHEN v.name IN ('attemptId','performedAction','resumeSameAttempt','authorizedCleanup','survivingAttemptId') THEN 'reference' ELSE 'selector' END,'name',v.name,'valueType',v.value_type,'value',v.value,'sourceRowCount',CASE WHEN v.value IS NULL THEN 0 ELSE 1 END,'sourceSha256',encode(digest(v.name||':'||v.value::text||':'||$2::text||':'||$4::text,'sha256'),'hex')) fact,v.ordinality FROM valued v WHERE v.value IS NOT NULL
 ), base AS (
  SELECT jsonb_build_object('readerSchemaVersion','REV869B-FACTS-v4','readerId','CP-L4','scope',jsonb_build_object('companyId','not-applicable-control-plane','targetInstanceSha256',$1,'leaseId',$2,'leaseVersion',$3,'operationId',$4,'scenarioExecutionId',$7,'subcaseId',$9,'stage',$8),'observedAtUtc',to_char(statement_timestamp() AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),'transactionBoundary','tx:'||txid_current_snapshot()::text||':'||$8||':'||$7::text,'facts',coalesce((SELECT jsonb_agg(fact ORDER BY ordinality) FROM fact_rows),'[]'::jsonb),'factCount',(SELECT count(*) FROM fact_rows)) payload
 )
 SELECT payload||jsonb_build_object('rawSha256',encode(digest(nexa.rev869b_canonical_json_v3(payload),'sha256'),'hex')) FROM base
 WHERE session_user='nexa_rev869b_control_plane_verifier' AND $1~'^[0-9a-f]{64}$' AND $2<>'00000000-0000-0000-0000-000000000000'::uuid AND $3>0 AND $4<>'00000000-0000-0000-0000-000000000000'::uuid AND $5<>'00000000-0000-0000-0000-000000000000'::uuid AND $7<>'00000000-0000-0000-0000-000000000000'::uuid AND $8 IN ('Before','After','Durable','Cleanup') AND $9~'^[A-Z][0-9]{2}:[a-z0-9-]{2,80}$' AND coalesce(cardinality($10),0)=(SELECT count(*) FROM requested) AND coalesce(cardinality($10),0)=(SELECT count(DISTINCT name) FROM requested) AND coalesce(cardinality($10),0)=(SELECT count(*) FROM fact_rows) $$;

CREATE FUNCTION nexa.rev869b_read_control_acl_facts_v4(instance_sha256 text,lease_id uuid,lease_version bigint,operation_id uuid,scenario_execution_id uuid,principal name,object_identity text,operation text,observation_stage text,subcase_id text,requested_facts text[]) RETURNS jsonb LANGUAGE sql SECURITY DEFINER VOLATILE SET search_path=pg_catalog,nexa AS $$
 ...
 ), snapshot AS (SELECT count(*) fact_count,count(*) FILTER(WHERE fact LIKE '%|PUBLIC|%') public_count,encode(digest(coalesce(string_agg(fact,chr(10) ORDER BY fact),''),'sha256'),'hex') sha FROM acl_facts), allowed(name,value_type) AS (VALUES ('pinMismatchCount','int64'),('verificationMismatchCount','int64'),('seededDeltaCount','int64'),('reportedDeltaSha256','sha256'),('protectedMutationCount','int64'),('cleanupFingerprint','sha256'),('controlObservedMinusExpectedCount','int64'),('seededDeltaSha256','sha256'),('baselineFingerprint','sha256')), requested AS (SELECT a.*,u.ordinality FROM unnest($11) WITH ORDINALITY u(name,ordinality) JOIN allowed a USING(name)), valued AS (
  SELECT r.*,CASE WHEN r.value_type='sha256' THEN to_jsonb(s.sha) WHEN r.name IN ('pinMismatchCount','verificationMismatchCount','controlObservedMinusExpectedCount') THEN to_jsonb(CASE WHEN (SELECT CatalogueSha256 FROM nexa.rev869b_control_plane_manifest LIMIT 1)=nexa.rev869b_control_plane_catalogue_fingerprint() THEN 0 ELSE 1 END) WHEN r.name IN ('seededDeltaCount','protectedMutationCount') THEN to_jsonb(CASE WHEN (SELECT CatalogueSha256 FROM nexa.rev869b_control_plane_manifest LIMIT 1)=nexa.rev869b_control_plane_catalogue_fingerprint() THEN 0 ELSE 1 END) ELSE NULL::jsonb END value FROM requested r CROSS JOIN snapshot s
 ), fact_rows AS (SELECT jsonb_build_object('kind',CASE WHEN v.name IN ('seededDeltaSha256','baselineFingerprint') THEN 'reference' ELSE 'selector' END,'name',v.name,'valueType',v.value_type,'value',v.value,'sourceRowCount',CASE WHEN v.value IS NULL THEN 0 ELSE 1 END,'sourceSha256',encode(digest(v.name||':'||v.value::text||':'||$6::text||':'||$7,'sha256'),'hex')) fact,v.ordinality FROM valued v WHERE v.value IS NOT NULL), base AS (
  SELECT jsonb_build_object('readerSchemaVersion','REV869B-FACTS-v4','readerId','CP-A4','scope',jsonb_build_object('companyId','not-applicable-control-plane','targetInstanceSha256',$1,'leaseId',$2,'leaseVersion',$3,'operationId',$4,'scenarioExecutionId',$5,'subcaseId',$10,'stage',$9),'observedAtUtc',to_char(statement_timestamp() AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),'transactionBoundary','tx:'||txid_current_snapshot()::text||':'||$9||':'||$5::text,'facts',coalesce((SELECT jsonb_agg(fact ORDER BY ordinality) FROM fact_rows),'[]'::jsonb),'factCount',(SELECT count(*) FROM fact_rows)) payload)
 SELECT payload||jsonb_build_object('rawSha256',encode(digest(nexa.rev869b_canonical_json_v3(payload),'sha256'),'hex')) FROM base WHERE session_user='nexa_rev869b_control_plane_verifier' AND $1~'^[0-9a-f]{64}$' AND $2<>'00000000-0000-0000-0000-000000000000'::uuid AND $3>0 AND $4<>'00000000-0000-0000-0000-000000000000'::uuid AND $5<>'00000000-0000-0000-0000-000000000000'::uuid AND length($6::text)>0 AND length($7)>0 AND length($8)>0 AND $9 IN ('Before','After','Durable','Cleanup') AND $10~'^[A-Z][0-9]{2}:[a-z0-9-]{2,80}$' AND coalesce(cardinality($11),0)=(SELECT count(*) FROM requested) AND coalesce(cardinality($11),0)=(SELECT count(DISTINCT name) FROM requested) AND coalesce(cardinality($11),0)=(SELECT count(*) FROM fact_rows) $$;
...
GRANT EXECUTE ON FUNCTION nexa.rev869b_read_lease(uuid),nexa.rev869b_read_nonterminal_leases(text),nexa.rev869b_read_lifecycle_evidence(uuid,uuid,uuid,uuid),nexa.rev869b_read_control_plane_acl_evidence(),nexa.rev869b_read_lifecycle_evidence_v2(text,uuid,text,text,uuid,uuid,uuid,bigint),nexa.rev869b_read_control_plane_acl_evidence_v2(text,text),nexa.rev869b_read_lifecycle_facts_v4(text,uuid,bigint,uuid,uuid,uuid,uuid,text,text,text[]),nexa.rev869b_read_control_acl_facts_v4(text,uuid,bigint,uuid,uuid,name,text,text,text,text,text[]),nexa.rev869b_control_plane_catalogue_fingerprint() TO nexa_rev869b_control_plane_verifier;
