...
const fixedCopyPath = "C:\\Users\\User\\AppData\\Local\\SESS NexaERP\\app\\InventoryERP_Software_REV858_MenuFixed.html";

const stamp = new Date().toISOString().replace(/[:.]/g, "-");
const appBackup = `${appPath}.bak-REV857-before-menu-fix-${stamp}`;
const serverBackup = `${serverPath}.bak-REV857-before-menu-fix-${stamp}`;

let html = fs.readFileSync(appPath, "utf8");
let server = fs.readFileSync(serverPath, "utf8");

fs.writeFileSync(appBackup, html, "utf8");
fs.writeFileSync(serverBackup, server, "utf8");

html = html.replace(/REV857/g, "REV858");
server = server.replace(/REV857/g, "REV858");

const marker = "<!-- SESS_REV858_MENU_LAYOUT_FIXED: desktop/sidebar menu width, icon alignment, single-scroll nav, clean search icon, and mobile drawer fix. -->";
if (!html.includes(marker)) {
  html = html.replace(/<title>SESS NexaERP - Software REV858<\/title>/, '<title>SESS NexaERP - Software REV858</title>\n  ' + marker);
}

const css = String.raw`
<style id="SESS_REV858_MENU_LAYOUT_FIXED">
  *, *::before, *::after { box-sizing: border-box; }
  html, body { overflow-x: hidden !important; }
  :root { --nav-width: 270px !important; --sess-sidebar-width: 270px; --sess-drawer-width: 280px; }

  body:not(.role-portal-view) #tabs,
  body:not(.role-portal-view) nav#tabs {
  ...
  .sess-menu-search-wrap { position: relative !important; width: 100% !important; }
  ...
  .sess-menu-search::placeholder { color: #64748B !important; opacity: 1 !important; }
  ...
  .menu-group.collapsed .menu-group-body { display: none !important; max-height: 0 !important; padding: 0 !important; overflow: hidden !important; }

  #tabs .menu-group-body button[data-tab], nav#tabs button[data-tab] {
    display: flex !important;
    align-items: center !important;
  ...
  #tabs .menu-group-body button[data-tab]::before, nav#tabs button[data-tab]::before, nav#tabs button[data-icon]::before {
  ...
  #tabs .menu-group-body button[data-tab]:hover, nav#tabs button[data-tab]:hover {
    background: #EEF4FF !important;
    border-color: #BFDBFE !important;
    color: #0B3A82 !important;
  }
  #tabs .menu-group-body button[data-tab].active, nav#tabs button[data-tab].active {
    min-height: 40px !important;
  ...
  #tabs .menu-group-body button[data-tab].active::before, nav#tabs button[data-tab].active::before {
  ...
  #tabs .menu-group.sess-menu-hidden, #tabs button[data-tab].sess-menu-hidden { display: none !important; }

  .sess-mobile-menu-toggle, .sess-mobile-menu-overlay, .sess-mobile-menu-close { display: none; }
  @media (max-width: 768px) {
    :root { --nav-width: 0px !important; --sess-sidebar-width: 0px; }
    body:not(.role-portal-view) #tabs, body:not(.role-portal-view) nav#tabs {
      top: 0 !important;
    ...
    body:not(.role-portal-view) main { margin-left: 0 !important; width: 100% !important; max-width: 100% !important; }
...
html = html.replace(/<style id="SESS_REV858_MENU_LAYOUT_FIXED">[\s\S]*?<\/style>\s*/g, "");
html = html.replace(/<\/head>/, css + "\n</head>");

const controls = '<button type="button" id="sessMobileMenuToggle" class="sess-mobile-menu-toggle" aria-label="Open ERP menu" aria-controls="tabs" aria-expanded="false">MENU</button>\n  <button type="button" id="sessMobileMenuClose" class="sess-mobile-menu-close" aria-label="Close ERP menu">X</button>\n  <div id="sessMobileMenuOverlay" class="sess-mobile-menu-overlay" aria-hidden="true"></div>\n';
...
const js = String.raw`
<script id="SESS_REV858_MENU_LAYOUT_FIXED_JS">
  (function () {
    const REV = "REV858";
    ...
    function bindMenuDrawer() {
      ...
      document.querySelectorAll("#tabs button[data-tab]").forEach(function (button) {
        if (button.dataset.rev858Bound === "1") return;
        button.dataset.rev858Bound = "1";
...
html = html.replace(/<script id="SESS_REV858_MENU_LAYOUT_FIXED_JS">[\s\S]*?<\/script>\s*/g, "");
html = html.replace(/<\/body>/, js + "\n</body>");

server = server.replace(
  /\/\/ SESS_REV856_CANONICAL_COMPANY_SELECTOR_GUARD: backend revision aligned with canonical company selector guard\./,
  "// SESS_REV858_MENU_LAYOUT_FIXED: backend revision aligned after focused sidebar menu layout fix.\n// SESS_REV856_CANONICAL_COMPANY_SELECTOR_GUARD: backend revision aligned with canonical company selector guard."
);
if (!server.includes("SESS_REV858_MENU_LAYOUT_FIXED")) {
  server = server.replace(
...
fs.writeFileSync(fixedCopyPath, html, "utf8");
fs.writeFileSync(serverPath, server, "utf8");

console.log("REV858 menu layout fixed from REV857 source.");
console.log("Backup:", appBackup);
console.log("Fixed copy:", fixedCopyPath);
