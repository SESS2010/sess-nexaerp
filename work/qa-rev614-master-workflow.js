...
const outPath = path.join(__dirname, "..", "outputs", "rev614_master_workflow_check.md");
...
const checks = [
  ["Revision is REV614 in installed UI", /Software REV614/.test(html), "Installed HTML visible revision is aligned."],
  ...
  ["Product Master is a real entry form", /id="productMasterForm"|productMasterForm/.test(html), "Expected separate finished-goods/product save form."],
  ...
  ["Service Master blocks duplicate asset number", /Duplicate asset|assetNumber.*already|serviceAssets\.some\([^)]*assetNumber/.test(serviceHandler), "Expected duplicate asset number guard in normal save handler."],
  ["Service Master blocks duplicate serial number", /Duplicate serial|serialNumber.*already|serviceAssets\.some\([^)]*serialNumber/.test(serviceHandler), "Expected duplicate serial number guard in normal save handler."],
...
const lines = [];
lines.push("# REV614 Master Workflow Check");
lines.push("");
...
lines.push("- Product Master is currently a REV613 dynamic linked view sourced from Item Master/Project Master data, not a separate finished-goods/product entry workflow.");
lines.push("- Service/Machine Master currently saves/updates records without a strict duplicate asset/serial guard in the normal save handler.");
